...
class AmazonProduct_Result extends AmazonProduct_Abstract implements IteratorAggregate {
    ...
    public function __construct( $code = null, $data = null, $request = null ) {
        ...
        if( $this->_data != null ) {
            ...
            $errors = array();
            $error = new AmazonProduct_Error();
            $error->set( "Code", "-1" );
            $error->set( "Message", "Unable to connect to Amazon Product API, please validate your Public and Secret Codes." );
            $errors.push( $error);
    ...
    public function parseXML( $xml ) {
        ...
        if( stripos($x->nodeName, "Error" ) !== false ) {
            $this->set( "IsValid", "False" );
